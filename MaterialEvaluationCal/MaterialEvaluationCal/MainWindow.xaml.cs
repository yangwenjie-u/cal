using MaterialEvaluationCal.page;
using System.Windows;

namespace MaterialEvaluationCal
{
    /// <summary>
    /// MainWindow.xaml 的交互逻辑
    /// </summary>
    public partial class MainWindow : Window
    {
        public MainWindow()
        {
            InitializeComponent();
        }

        private void Button_Click_1(object sender, RoutedEventArgs e)
        {
            JGG f2 = new JGG();
            f2.ShowDialog();
        }

        private void Button_Click(object sender, RoutedEventArgs e)
        {
            GHJ hj = new GHJ();
            hj.ShowDialog();
        }

        private void Button_Click_2(object sender, RoutedEventArgs e)
        {
            GLJ gLJ = new GLJ();
            gLJ.ShowDialog();
        }
        private void Button_Click_3(object sender, RoutedEventArgs e)
        {
            HSZ hSZ = new HSZ();
            hSZ.ShowDialog();
        }
        private void Button_Click_4(object sender, RoutedEventArgs e)
        {
            QK2 qk2 = new QK2();
            qk2.ShowDialog();
        }

        private void Button_Click_5(object sender, RoutedEventArgs e)
        {
            //KS ks2 = new KS();
            //ks2.ShowDialog();
        }
        private void Button_Click_6(object sender, RoutedEventArgs e)
        {
            GYC gyc = new GYC();
            gyc.ShowDialog();
        }
        private void Button_Click_7(object sender, RoutedEventArgs e)
        {
            TZY tZY = new TZY();
            tZY.ShowDialog();
        }
        private void Button_Click_8(object sender, RoutedEventArgs e)
        {
            GG gG = new GG();
            gG.ShowDialog();
        }
        //private void Button_Click_9(object sender, RoutedEventArgs e)
        //{
        //    SJX sjx = new SJX();
        //    sjx.ShowDialog();
        //}

        private void Button_Click_10(object sender, RoutedEventArgs e)
        {
            MC mc = new MC();
            mc.ShowDialog();
        }

        private void Button_Click_11(object sender, RoutedEventArgs e)
        {
            JBS jbs = new JBS();
            jbs.ShowDialog();
        }

        private void Button_Click_12(object sender, RoutedEventArgs e)
        {
            JMC jMC = new JMC();
            jMC.ShowDialog();
        }
        private void Button_Click_13(object sender, RoutedEventArgs e)
        {
            JZBL jZBL = new JZBL();
            jZBL.ShowDialog();
        }
        private void Button_Click_14(object sender, RoutedEventArgs e)
        {
            HPB hpb = new HPB();
            hpb.ShowDialog();
        }


        private void Button_Click_15(object sender, RoutedEventArgs e)
        {
            SN sn = new SN();
            sn.ShowDialog();
        }

        private void Button_Click_16(object sender, RoutedEventArgs e)
        {
            TG tG = new TG();
            tG.ShowDialog();
        }

        private void Button_Click_17(object sender, RoutedEventArgs e)
        {
            LNG lNG = new LNG();
            lNG.ShowDialog();
        }

        private void Button_Click_18(object sender, RoutedEventArgs e)
        {
            SPB spb = new SPB();
            spb.ShowDialog();
        }

        private void Button_Click_19(object sender, RoutedEventArgs e)
        {
            DXL dxl = new DXL();
            dxl.ShowDialog();
        }

        private void Button_Click_20(object sender, RoutedEventArgs e)
        {
            ZSJ zsj = new ZSJ();
            zsj.ShowDialog();
        }

        private void Button_Click_21(object sender, RoutedEventArgs e)
        {
            HSA hsa = new HSA();
            hsa.ShowDialog();
        }

        private void Button_Click_22(object sender, RoutedEventArgs e)
        {
            HNT obj = new HNT();
            obj.ShowDialog();
        }
        private void Button_Click_23(object sender, RoutedEventArgs e)
        {
            HD hd = new HD();
            hd.ShowDialog();
        }
        private void Button_Click_24(object sender, RoutedEventArgs e)
        {
            PZJ pZJ = new PZJ();
            pZJ.ShowDialog();
        }
        private void Button_Click_25(object sender, RoutedEventArgs e)
        {
            GSF hkz = new GSF();
            hkz.ShowDialog();
        }

        private void Button_Click_26(object sender, RoutedEventArgs e)
        {
            GGF hkz = new GGF();
            hkz.ShowDialog();
        }

        private void Button_Click_27(object sender, RoutedEventArgs e)
        {
            QK3 qK3 = new QK3();
            qK3.ShowDialog();
        }

        private void Button_Click_28(object sender, RoutedEventArgs e)
        {
            ZDZD zDZD = new ZDZD();
            zDZD.ShowDialog();
        }

        private void Button_Click_29(object sender, RoutedEventArgs e)
        {
            GHJ gHJ = new GHJ();
            gHJ.ShowDialog();
        }
        private void Button_Click_30(object sender, RoutedEventArgs e)
        {
            FDJ fDJ = new FDJ();
            fDJ.ShowDialog();
        }

        private void Button_Click_31(object sender, RoutedEventArgs e)
        {
            GLJ gLJ = new GLJ();
            gLJ.ShowDialog();
        }
        private void Button_Click_33(object sender, RoutedEventArgs e)
        {

        }
        private void Button_Click_32(object sender, RoutedEventArgs e)
        {
            GYF gYF = new GYF();
            gYF.ShowDialog();
        }

        private void Button_Click_34(object sender, RoutedEventArgs e)
        {
            CreateTable createTable = new CreateTable();
            createTable.ShowDialog();
        }
        private void Button_Click_GLF(object sender, RoutedEventArgs e)
        {
            GLF gLF = new GLF();
            gLF.ShowDialog();
        }
        private void Button_Click_35(object sender, RoutedEventArgs e)
        {
            Qsf xC = new Qsf();
            xC.ShowDialog();
        }
        private void Button_Click_XC(object sender, RoutedEventArgs e)
        {
            XC xC = new XC();
            xC.ShowDialog();
        }

        private void Button_Click_WLB(object sender, RoutedEventArgs e)
        {
            WLB wLB = new WLB();
            wLB.ShowDialog();
        }
        private void Button_Click_FM(object sender, RoutedEventArgs e)
        {
            FM fM = new FM();
            fM.ShowDialog();
        }

        private void Button_Click_AQW(object sender, RoutedEventArgs e)
        {
            AQW aQW = new AQW();
            aQW.ShowDialog();
        }

        private void Button_Click_YKJ(object sender, RoutedEventArgs e)
        {
            YKJ yKJ = new YKJ();
            yKJ.ShowDialog();
        }

        private void Button_Click_KF(object sender, RoutedEventArgs e)
        {
            KF kF = new KF();
            kF.ShowDialog();
        }

        private void Button_Click_XJL(object sender, RoutedEventArgs e)
        {
            XJL xJL = new XJL();
            xJL.ShowDialog();
        }
        private void Button_Click_QSF(object sender, RoutedEventArgs e)
        {
            Qsf qsf = new Qsf();
            qsf.ShowDialog();
        }

        private void Button_Click_AW(object sender, RoutedEventArgs e)
        {
            AW aW = new AW();
            aW.ShowDialog();
        }
        private void Button_Click_HT1(object sender, RoutedEventArgs e)
        {
            HT1 ht1 = new HT1();
            ht1.ShowDialog();
        }
        private void Button_Click_CJL(object sender, RoutedEventArgs e)
        {
            CJL cJL = new CJL();
            cJL.ShowDialog();
        }
        //外加剂

        private void Button_Click_37(object sender, RoutedEventArgs e)
        {
            WJJ aW = new WJJ();
            aW.ShowDialog();
        }

        private void Button_Click_JPH(object sender, RoutedEventArgs e)
        {
            JPH jPH = new JPH();
            jPH.ShowDialog();
        }
        private void Btn_ywj_Click(object sender, RoutedEventArgs e)
        {
            FormYWJ form = new FormYWJ();
            form.ShowDialog();
        }
        private void Button_Click_LSL(object sender, RoutedEventArgs e)
        {
            LSL lSL = new LSL();
            lSL.ShowDialog();
        }

        private void Btn_TBH_Click(object sender, RoutedEventArgs e)
        {
            TBH tbh = new TBH();
            tbh.ShowDialog();
        }

        private void Button_Click_36(object sender, RoutedEventArgs e)
        {
            ZX zx = new ZX();
            zx.ShowDialog();
        }

        private void Button_Click_39(object sender, RoutedEventArgs e)
        {
            TSZ tsz = new TSZ();
            tsz.ShowDialog();
        }
    }
}