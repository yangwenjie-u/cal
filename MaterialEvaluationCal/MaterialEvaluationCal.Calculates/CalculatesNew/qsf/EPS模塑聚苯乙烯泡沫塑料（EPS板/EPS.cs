...
namespace Calculates
{
    public class EPS:BaseMethods
    {
        public void Calc()
        {
            ...
            var mjcjg = "不合格";
            var jsbeizhu = "";
            var SItem = data["S_EPS"];
            var BCRDJ = dataExtra["BZ_EPS_DJ"];
            var YZSKB = dataExtra["BZ_YZSKB"];
            //var DRXSDJ = dataExtra["BZ_BBDRXS"];
            var MItem = data["M_EPS"];
            ...
            if (!data.ContainsKey("M_EPS"))
            {
                data["M_EPS"] = new List<IDictionary<string, string>>();
            }
            if (MItem == null)
            {
            ...
            foreach (var sItem in SItem)
            {
                ...
                {
                    jcxmCur = "吸水率";
                    string rz1 = calc_PB(mItem["G_XSL"], sItem["W_XSL"], true);
                    if (rz1 == "符合")
                    {
                        mItem["HG_XSL"] = "合格";
                        mFlag_Hg = true;
