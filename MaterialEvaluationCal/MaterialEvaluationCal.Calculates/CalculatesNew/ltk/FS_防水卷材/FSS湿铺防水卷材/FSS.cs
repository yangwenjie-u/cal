...
namespace Calculates
{
    public class FSS : BaseMethods
    {
        public void Calc()
        {
            ...
            var mjcjg = "不合格";
            var jsbeizhu = "";
            var SItems = data["S_FSS"];
            var MItem = data["M_FSS"];
            var mrsDjs = dataExtra["BZ_FS_DJS"];
            var mAllHg = true;
            int mbhggs = 0;
            var jcxmBhg = "";
            var jcxmCur = "";
            var jcxm = "";
            var QBZZZD = "";
            if (!data.ContainsKey("M_FSS"))
            {
                data["M_FSS"] = new List<IDictionary<string, string>>();
            }
            if (MItem == null)
            {
