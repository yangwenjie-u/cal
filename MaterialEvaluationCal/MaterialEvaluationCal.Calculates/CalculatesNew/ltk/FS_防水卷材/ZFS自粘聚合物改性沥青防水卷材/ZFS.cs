...
namespace Calculates
{
    public class ZFS : BaseMethods
    {
        public void Calc()
        {
            ...
            var mjcjg = "不合格";
            var jsbeizhu = "";
            var SItems = data["S_ZFS"];
            var MItem = data["M_ZFS"];
            var mrsDjs = dataExtra["BZ_FS_DJZ"];
            var mAllHg = true;
            int mbhggs = 0;
            var jcxmBhg = "";
            var jcxmCur = "";
            var jcxm = "";
            var QBZZZD = "";
            if (!data.ContainsKey("M_ZFS"))
            {
                data["M_ZFS"] = new List<IDictionary<string, string>>();
            }
            if (MItem == null)
            {
            ...
            foreach (var sItem in SItems){
                ...
                sItem["G_H_SDLL"] = mrsDj["G_HXLL"];
                sItem["G_Z_SDLL"] = mrsDj["G_ZXLL"];
               
                sItem["G_H_MDSCL"] = mrsDj["G_HXZDLYSL"];
                sItem["G_Z_MDSCL"] = mrsDj["G_ZXZDLYSL"];
                ...
                else
                {
                    ...
                    if (sItem["JCHD"].Contains("3"))
                    {
                        sItem["G_H_SDLL"] = "≥600";
                        sItem["G_Z_SDLL"] = "≥600";
                    }
                    if (sItem["JCHD"].Contains("4"))
                    {
                        sItem["G_H_SDLL"] = "≥800";
                        sItem["G_Z_SDLL"] = "≥800";
                    }
                ...
                if (jcxm.Contains("、拉伸性能、")){
                    ...
                    for(int i = 1; i<6; i++)
                    {
                        HXSDLLSUM =GetSafeDouble( sItem["H_SDLL" + i]) + HXSDLLSUM;
                    }
                    sItem["H_PJSDLL"] = Round(HXSDLLSUM / 5, 0).ToString();
                    sItem["HG_H_SDLL"]= IsQualified(sItem["G_H_SDLL"], sItem["H_PJSDLL"], false);
                    if (sItem["HG_H_SDLL"] == "不合格")
                    {
                        mAllHg = false;
                    ...
                    for (int i = 1; i < 6; i++)
                    {
                        ZXSDLLSUM = GetSafeDouble(sItem["Z_SDLL" + i]) + ZXSDLLSUM;
                    }
                    sItem["Z_PJSDLL"] = Round(ZXSDLLSUM / 5, 0).ToString();
                    sItem["HG_Z_SDLL"] = IsQualified(sItem["G_Z_SDLL"], sItem["Z_PJSDLL"], false);
                    if (sItem["HG_Z_SDLL"] == "不合格")
                    {
                        mAllHg = false;
                        jcxmBhg += jcxmBhg.Contains(jcxmCur) ? "" : jcxmCur + "、";
                    }
                    ...
                    else { sItem["HG_H_LSXX"] = "合格"; sItem["NG_HXLSXXJCJG"] = "胶层与高分子膜或胎基无分离现象"; }
                    #endregion
                    #region 纵向拉伸现象
                    jcxmCur = "纵向拉伸现象";
                    int ZXbhggs = 0;
                    ...
                    else { sItem["HG_Z_LSXX"] = "合格"; sItem["NG_ZXLSXXJCJG"] = "胶层与高分子膜或胎基无分离现象"; }
                    #endregion
                }
                else 
                { 
                ...
                if (jcxm.Contains("、耐热性、")) {
                    jcxmCur = "耐热性";
                    int NRbhggs = 0;
                    List<double> HDJL = new List<double>();
                    HDJL.Add(GetSafeDouble(sItem["NRHDJL1"]));
                    HDJL.Add(GetSafeDouble(sItem["NRHDJL2"]));
                    HDJL.Add(GetSafeDouble(sItem["NRHDJL3"]));
                    HDJL.Sort();
                    sItem["NRHDZDJL"] = HDJL[2].ToString();
                    for (int i = 1; i < 4; i++)
                    {
                        if (sItem["NRXX" + i] != "无流淌、滴落" || HDJL[2]>2)
                        {
                            NRbhggs = NRbhggs + 1;
                        }
                    }
