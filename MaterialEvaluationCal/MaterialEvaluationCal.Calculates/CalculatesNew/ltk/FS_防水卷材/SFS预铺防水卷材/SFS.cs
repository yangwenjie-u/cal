...
namespace Calculates
{
    public class SFS : BaseMethods
    {
        public void Calc()
        {
            ...
            var mjcjg = "不合格";
            var jsbeizhu = "";
            var SItems = data["S_SFS"];
            var MItem = data["M_SFS"];
            var mrsDjs = dataExtra["BZ_FS_DJY"];
            var mAllHg = true;
            int mbhggs = 0;
            var jcxmBhg = "";
            var jcxmCur = "";
            var jcxm = "";
            var QBZZZD = "";
            if (!data.ContainsKey("M_SFS"))
            {
                data["M_SFS"] = new List<IDictionary<string, string>>();
            }
            if (MItem == null)
            {
            ...
            foreach (var sItem in SItems){
                ...
                sItem["G_H_MDSCL"] = mrsDj["G_HXMDSCL"];
                sItem["G_Z_MDSCL"] = mrsDj["G_ZXMDSCL"];
                ...
                if (jcxm.Contains("、拉伸性能、")){
                    ...
                    for (int i = 1; i < 6; i++)
                    {
                        HXSDLLSUM = GetSafeDouble(sItem["Z_SDLL" + i]) + HXSDLLSUM;
                    }
                    sItem["Z_PJSDLL"] = Round(HXSDLLSUM / 5, 0).ToString();
                    ...
                    else { sItem["HG_H_LSXX"] = "合格"; sItem["NG_HXLSXXJCJG"] = "胶层与主体材料或胎基无分离现象"; }
                    #endregion
                    #region 纵向拉伸现象
                    jcxmCur = "纵向拉伸现象";
                    int ZXbhggs = 0;
                    ...
                    else { sItem["HG_Z_LSXX"] = "合格"; sItem["NG_ZXLSXXJCJG"] = "胶层与主体材料或胎基无分离现象"; }
                    #endregion
                }
                else 
                { 
