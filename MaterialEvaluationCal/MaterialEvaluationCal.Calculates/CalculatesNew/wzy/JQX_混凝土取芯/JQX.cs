...
namespace Calculates
{
    public class JQX : BaseMethods
    {
        public void Calc()
        {
            ...
            List<string> kyqdArray = new List<string>();
            ...
            #region  集合取值
            var data = retData;
            var mrsDj = dataExtra["BZ_JQX_DJ"];
            var mrsHsb = dataExtra["BZ_QXHSB"];
            var mrsQjb = dataExtra["BZ_QXQJXS"];
            var MItem = data["M_JQX"];
            var SItem = data["S_JQX"];
            int zxgjs = 0;
            int hggjs = 0;
            #endregion
            ...
            foreach (var sitem in SItem)
            {
                ...
                {
                    //高径比计算
                    zj = (GetSafeDouble(sitem["ZJ" + i]) / 100).ToString();
                    g = (GetSafeDouble(sitem["GD" + i]) / 100).ToString();
                    sitem["GJB"+i] = g + ":" + zj;

                    mZj = 0;
                    mMj = 0;
                    mZj = Conversion.Val(sitem["ZJ" + i]);
                    mMj = 3.14159 * (mZj / 2) * (mZj / 2);
                    if (string.IsNullOrEmpty(sitem["KYHZ" + i]) || GetSafeDouble(sitem["KYHZ" + i]) == 0)
                    {
                        mSFwc = false;
                    }
                    if (mMj != 0)
                    {
                        sitem["MJ" + i] = Round(mMj, 0).ToString();
                        //计算单组的抗压强度,并进行合格判断
                        //sitem["KYHZ1"] = sitem["KYHZ1"];
                        sitem["KYQD" + i] = Round(1000 * GetSafeDouble(sitem["KYHZ" + i]) / mMj, 1).ToString("0.0");
                    }
                    else
                    {
                        sitem["KYQD" + i] = "0";
                    }
                    kyqdArray.Add(sitem["KYQD" + i]);
                }
                MItem[0]["BGZS_G"] = ((int)GetSafeDouble(MItem[0]["BGZS_G"]) + 1).ToString();
                kyqdArray.Sort();
                //强度代表值
                sitem["KYPJ"] = kyqdArray[0];
                sitem["DDSJQD"] = Math.Round(GetSafeDouble(sitem["KYPJ"]) / mSz * 100, 0).ToString("0");
                //总构件数
                zxgjs++;
                //符合构件数
                if (GetSafeDouble(sitem["DDSJQD"]) >= 100)
                {
                    hggjs++;
                }
            }
            MItem[0]["ZXGJS"] = zxgjs.ToString();
            MItem[0]["HGGJS"] = hggjs.ToString();
            //合格率
            MItem[0]["HGL"] = Math.Round(GetSafeDouble(MItem[0]["HGGJS"]) / GetSafeDouble(MItem[0]["ZXGJS"]) * 100, 2).ToString("0.00");
            #region 
            ///*
