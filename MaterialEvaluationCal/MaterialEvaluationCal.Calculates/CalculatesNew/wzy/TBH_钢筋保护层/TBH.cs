        ...
        public void Calc()
        {
            ...
            foreach (var sitem in SItem)
            {
                ...
                if (flag)
                {
                    ...
                    yxpc2 = int.Parse(sitem["YXPC"].Substring(sitem["YXPC"].IndexOf("～") + 1));
                }
            ...
            if (!sign)
            {
                if (falg_b && !falg_l)
                {
                    if (GetSafeDouble(MItem[0]["BHGL"]) >= 90)
                    {
                        ...
                        MItem[0]["JCJG_B"] = "合格";
                        MItem[0]["JCJG_L"] = "不合格";
                        MItem[0]["JCJGMS"] = "以上所检项中，梁不符合" + MItem[0]["PDBZ"] + "标准要求。";
                    }
                    ...
                    else
                    {
                        ...
                        MItem[0]["JCJG_B"] = "不合格";
                        MItem[0]["JCJG_L"] = "不合格";
                        MItem[0]["JCJGMS"] = "以上所检项不符合" + MItem[0]["PDBZ"] + "标准要求。";
                    }
                }
                else if (!falg_b && falg_l)
                {
                    if (GetSafeDouble(MItem[0]["LHGL"]) >= 90)
                    {
                        ...
                        MItem[0]["JCJG_B"] = "合格";
                        MItem[0]["JCJG_L"] = "不合格";
                        MItem[0]["JCJGMS"] = "以上所检项中，板不符合" + MItem[0]["PDBZ"] + "标准要求。";
                    }
                    else if (GetSafeDouble(MItem[0]["BHGL"]) >= 80)
                    {
                        mAllHg = false;
                ...
                else
                {
                    if (GetSafeDouble(MItem[0]["ZCGJB"]) > 0 && GetSafeDouble(MItem[0]["ZCGJL"]) > 0)
                    {
                        if (GetSafeDouble(MItem[0]["BHGL"]) >= 90 && GetSafeDouble(MItem[0]["LHGL"]) >= 90)
                        {
                            ...
                            MItem[0]["JCJG_B"] = "合格";
                            MItem[0]["JCJG_L"] = "合格";
                            MItem[0]["JCJGMS"] = "以上所检项符合" + MItem[0]["PDBZ"] + "标准要求。";
                        }
                        ...
                        else if (GetSafeDouble(MItem[0]["BHGL"]) >= 80 && GetSafeDouble(MItem[0]["LHGL"]) >= 80)
                        {
                            ...
                            MItem[0]["JCJG_B"] = "复试";
                            MItem[0]["JCJG_L"] = "复试";
                            MItem[0]["JCJGMS"] = "以上所检项中，板、梁不符合" + MItem[0]["PDBZ"] + "标准要求。";
                        }
                        else if (GetSafeDouble(MItem[0]["BHGL"]) < 80 && GetSafeDouble(MItem[0]["LHGL"]) >= 90)
                        {
                            mAllHg = false;
            ...
            MItem[0]["JCJGMS"] = flag ? MItem[0]["JCJGMS"] : "各构件实测结果见后页。";
            string mjgsm = string.Empty;
            mjgsm = flag ? mjgsm : "各构件实测结果见后页。";
            //if ((GetSafeDateTime(MItem[0]["JYRQ"]) - GetSafeDateTime("2015-12-18")).Days >= 0)
            //{
            //    MItem[0]["WHICH"] = "bgtbh_8";
            //    if (mMore && mmp)
            //        MItem[0]["WHICH"] = MItem[0]["WHICH"] + "、bgtbh_15";
            //    else if (mMore && !mmp)
            //        MItem[0]["WHICH"] = MItem[0]["WHICH"] + "、bgtbh_14";
            //    else
            //    {
            //        if (bgzs == 1)
            //            MItem[0]["WHICH"] = MItem[0]["WHICH"] + "、bgtbh_5";
            //        else if (bgzs == 2)
            //            MItem[0]["WHICH"] = MItem[0]["WHICH"] + "、bgtbh_6";
            //        else
            //            MItem[0]["WHICH"] = MItem[0]["WHICH"] + "、bgtbh_4";
            //    }
            //}
            ...
            //    {
            //        string bgfjs = (100 + i - 1).ToString();
            //        MItem[0]["WHICH"] = MItem[0]["WHICH"] + "、bgtbh_" + bgfjs;
            //    }
            //}
            //主表总判断赋值
            if (mAllHg)
