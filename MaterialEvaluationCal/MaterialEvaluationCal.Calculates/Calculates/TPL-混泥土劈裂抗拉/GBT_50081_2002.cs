...
namespace Calculates
{
    public class TPL : BaseMethods
    {
        public static bool Calc(IDictionary<string, IList<IDictionary<string, string>>> dataExtra, ref IDictionary<string, IDictionary<string, IList<IDictionary<string, string>>>> retData, ref string err)
        {
            ...
            var fistExtra = dataExtra["BZ_TPL_DJ"].FirstOrDefault();
            foreach (var JCXMitem in inspectionItems)
            {
                var tableItems = retData[JCXMitem]["S_TPL"];
                var s_tableItem = retData[JCXMitem]["S_BY_RW_XQ"];
                var ExtraTable = dataExtra["BZ_TPL_DJ"];
                foreach (var rowData in tableItems)//单行数据进行验证 key 字段名 val值
                {
                    ...
                    if (!dataExtra.ContainsKey("BZ_TPL_DJ"))
                    {
                        throw new Exception("【BZ_TPL_DJ】 表数据不存在");
                    }
                    var extraItem = ExtraTable.FirstOrDefault(u => (u.ContainsKey("MC") && u.Values.Contains(rowData["GGXH"])));
                    if (extraItem == null)
                    {
                        //依据不详
                        rwxqRow["JCJG"] = "依据不详";
                    ...
                    {
                        case "劈裂抗拉":
                            double Sjcc = GetSafeDouble(extraItem["SJCC"]), 
                                qdVal = 0, 
                                Sjcc1 = GetSafeDouble(rowData["SJCC1"]) == 0 ? Sjcc : GetSafeDouble(rowData["SJCC1"]),
                                mj = Sjcc* Sjcc1;

                            if(mj == 0 || GetSafeDouble(rowData["HSXS"]) == 0)
                            {
                                rowData["JCJG"] = "不合格";
                            ...
                            for (int i = 1;i<4;i++)
                            {
                                ...
                                qdVal = Math.Round(GetSafeDouble(rowData["KYHZ" + i]) * 1000 * 0.637 / mj, 2);
